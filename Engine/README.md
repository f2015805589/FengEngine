...
> 引擎文档根：总览、技术介绍与 Harness 项目守则。模块索引见 Engine/docs/README.md。

## 简介

FEngine 是一个基于 DirectX 12 从零构建的实时渲染引擎，采用 C++ 开发，面向学习和实验现代图形渲染技术。

## Harness：Agent 项目守则（强制）

### 1. 适用范围与效力

本章节适用于所有在本仓库 worktree 中运行的 Agent（以及人工维护者），是参与 Harness 任务的强制行为准则。本章节要求优先于任何个人习惯或便利性操作；任何违反守则的行为都必须立即停止并上报，不得擅自绕过。

### 2. 分支纪律

- 禁止在 main/master 上做任何修改
- 每次 agent 运行第一步必须执行 scripts/guard.ps1
- guard 拒绝时立即停止，禁止绕过（如 --no-verify、stash 后改主分支等）
- 工作分支命名约定：feature/<主题> 或 <主题>（如 管线优化）

### 3. 文档同步义务

- 完成/修改功能 → 必须更新对应模块 md（Engine/docs/01-Rendering/... 等）
- 修改公共接口、资产格式、Pass 行为 → 同步更新 Engine/README.md 模块描述
- 新增模块 → 新建 md 并注册到 Engine/docs/README.md 索引

### 4. 阅读导航协议（强制读取顺序）

1) Engine/README.md（总览与守则）
2) Engine/docs/README.md（模块索引）
3) 对应模块 md（导航与设计意图）
4) 源码（细节）

- md 是权威导航，源码是细节；禁止跳过 md 直接读源码

### 5. 提交流程（提交前必须自查清单：guard 通过、对应 md 已更新、无 main 分支操作）
...
- **SsgiPass**：屏幕空间全局光照（Screen Space Global Illumination），基于蒙特卡洛屏幕空间光线追踪实现间接漫反射照明。在法线半球内进行余弦加权采样，将视图空间射线投影到屏幕空间并以透视校正深度插值逐像素步进，命中表面后采样 Albedo 并计算距离衰减。渲染管线由多个子 Pass 构成：SSGI Raymarch（低分辨率）→ Upsample（深度感知升采样）→ Cross-Bilateral Blur（横向 + 纵向分离模糊，全分辨率）。输出 RGBA16F 纹理，RGB 为间接光颜色，A 通道为射线命中率权重。合成阶段利用命中率在 SSGI 与 IBL 之间 lerp 混合，未命中的射线由 IBL 环境光补充。支持可调节的方向数、步进数、追踪半径、强度和分辨率缩放参数。关闭时自动回退到黑色纹理（无间接光贡献）。

  **分辨率优化**：SSGI 支持三档分辨率缩放（Full 1/1、Half 1/2、Quarter 1/4），默认使用 Quarter 四分之一分辨率渲染以提升性能。为保证低分辨率下的视觉质量，实现了自适应参数调整机制：
  - **半径缩放**：低分辨率下自动缩小追踪半径（半分辨率 ÷2.6，四分之一分辨率 ÷3），避免过度采样导致的噪点和伪影。
  - **强度补偿**：同时提升 GI 强度（半分辨率 ×1.15，四分之一分辨率 ×1.3）以补偿半径减小带来的亮度损失。
  - **深度感知升采样**：使用双线性插值结合深度权重将低分辨率 SSGI 升采样到全分辨率，保持边缘细节。
  - **时域累积降噪**：集成 Temporal Accumulation，利用 Motion Vector 进行历史帧重投影，配合 3×3 邻域颜色裁剪和深度匹配，实现高达 95% 的帧间混合率，大幅降低蒙特卡洛采样噪点。支持快速运动检测和自适应混合权重。

  这套机制在保持视觉质量的同时，半分辨率可节省约 75% 的光线追踪开销，四分之一分辨率可节省约 93% 开销。
...
- **SSGI 调试面板**：支持开关 GI 模式（Off / SSGI），实时调节方向数、步进数、追踪半径、强度、分辨率缩放（Full/Half/Quarter）等参数。
